### Green Ampt with Redistribution + Overland Flow - MAIN ###
##By: Michelle Ives ###
## Fall 2023 ###

#calling functions from GAR_Function.R
#calling parameters from Soil_Parameters.R
source('~/Desktop/Data and R Scripts/GAR_Function.R')
source('~/Desktop/Data and R Scripts/Soil_Parameters.R')

#### CALCULATE OVERLAND FLOW INPUT 
...
#calculate overland flow using the following rules:
...
# Rainfall event = period of time where the input rate is large than the saturated hydraulic conductivity
...
#wetting front depth for 6 wetting fronts
Z1<-Z2<-Z3<-Z4<-Z5<-Z6<-NA
#soil moisture for 6 wetting fronts
SM_0_1<-SM_0_2<-SM_0_3<-SM_0_4<-SM_0_5<-SM_0_6<-NA
#cumulative infiltration depth for 6 wetting fronts
F1<-F2<-F3<-F4<-F5<-F6<-NA
#infiltration rate
f<-NA
#actual evapotranspiration (can also just use ET)
AET<-NA
#correction factor, will use later
N_r1<-NA
T_r1<-NA
N_r2<-NA
T_r2<-NA
...
#rename first column to make sure it matches event output data
colnames(GAR_Output_Data)[1] <- "DateTime"

##### converting potential evapotranspiration (PET) to actual evapotranspiration (AET) #####
#can use either PET or AET
#calculate h_50
wilting_point
FC
...
#using FC as the initial SM, but can use WP.
...
######## DEFINING NR and TR for each wetting front for Correction Factor #####
